...
imageUrl: /images/aws-data-warehouse.png
link: "https://github.com/shrikantnaidu/Data-Warehousing-with-AWS"
tags:
  - AWS Redshift
  - S3
  - ETL
...
**staging_events** - Staging table for events data from S3

**staging_songs** - Staging table for songs data from S3
...
The `COPY` command is a powerful feature of Amazon Redshift that allows for efficient loading of large datasets from Amazon S3 into Redshift tables. It is optimized for high throughput and can load data in parallel.
...
- **FROM** - Specifies the S3 bucket and path where the data files are located
- **IAM_ROLE** - The IAM role that has permission to access the S3 bucket
- **FORMAT AS JSON** - Indicates the format of the data being loaded

---

### Redshift Capabilities

Amazon Redshift is a fully managed, petabyte-scale data warehouse service in the cloud. Key capabilities include:

- **Scalability** - Easily scale your data warehouse up or down based on your needs
- **Performance** - Uses columnar storage and data compression to improve query performance
- **Integration** - Seamlessly integrates with various AWS services, including S3
- **Security** - Provides robust security features, including encryption and IAM roles
...
Create an IAM role with permissions to access S3 and Redshift. Note the IAM role ARN for use in the `COPY` command.
...
Create a `dwh.cfg` file with your AWS credentials and Redshift cluster details:
...
- Designed an optimized star schema for efficient querying of music streaming data
- Built a robust ETL pipeline that successfully processes and transforms JSON data
- Implemented data validation and quality checks throughout the pipeline
- Created a queryable database that enables complex analysis of user listening patterns
- Achieved efficient data loading with minimal duplicate records
- Implemented error handling and logging for pipeline monitoring
...
**AWS S3**
- Data lake storage for raw JSON files
- Source for ETL pipeline
